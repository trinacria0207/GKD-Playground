...
def test(model, test_inputs, test_labels):
    ...
    with open('res.txt', 'w') as f:
        f.write('Acc:'+str(accuracy.numpy())+"\n")
        f.write('F1 Score:'+str(f1.numpy())+"\n")
...
def main():
    ...
    with open(vocab_fp) as f:
        vocab = json.load(f)
    train_data = np.load(train_data_fp).astype(np.int64) #(N, window, 2)
    train_labels = np.load(train_labels_fp).astype(np.int64) #(N,)
    test_data = np.load(test_data_fp).astype(np.int64)
    test_labels = np.load(test_labels_fp).astype(np.int64)
